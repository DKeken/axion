...
```
Traefik Gateway → Next.js Frontend → NestJS Microservices → Kafka/Redis/PostgreSQL
```

**Компоненты платформы:**

1. **Traefik Gateway** — Load balancing, SSL termination, Rate limiting
2. **Next.js Frontend** — React 19 RSC, Better Auth, SSE, React Flow
3. **NestJS Microservices:**
   - Gateway Service — Service Discovery (Redis), routing
...
- Service Discovery (Redis): регистрация сервисов с TTL 30 секунд
