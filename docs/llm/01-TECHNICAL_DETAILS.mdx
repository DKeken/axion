...
2. **Network Level** — отдельные Docker сети для каждого проекта
...
- **HTTP RPC** — для межсервисного общения через внутренний IP Docker Swarm
...
├── config/              # Конфигурация
├── messaging/           # HTTP RPC клиент/сервер
