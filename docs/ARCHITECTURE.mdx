...
│                                                                       │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │  Backend: NestJS Microservices + Traefik                     │  │
│  │  ┌─────────────────────────────────────────────────────────┐  │  │
│  │  │  Graph Service                                          │  │  │
...
5. Генерация Всего Проекта
   ...
   1. Генерит общие файлы проекта:
      ...
      **docker-compose.yml:**
      ...
      - Инфраструктура (PostgreSQL, Redis) если нужна
      - Сети из edges
...
| --------------------- | ----------------------------------------- | ---------------------------------------- |
| **Retry**             | Exponential backoff для повторных попыток | Внешние зависимости (БД, HTTP API)       |
...
for (const edge of outgoingEdges) {
  ...
  // 3. Генерит HTTP RPC client, который вызывает сервис по адресу внутри Docker/Swarm:
  //   baseUrl = process.env[`${TARGET_SERVICE}_URL`] ?? `http://${targetServiceName}:3000`
  //   POST `${baseUrl}/rpc/<pattern>`
}
...
10. **SaaS на NestJS Microservices** - платформа использует NestJS + Kafka + Protobuf + Better Auth + Traefik (edge routing)
...
| **Edge routing** | Traefik                        | Traefik (на стороне деплоя)    |
| **Event Bus**    | Kafka                          | HTTP RPC (Docker Swarm)        |
