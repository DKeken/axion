...
- `main-entry.md` → `src/main.ts` (`ComponentType.MAIN_ENTRY`)
- `rabbitmq-server.md` → `src/messaging/server.ts` (`ComponentType.RABBITMQ_SERVER`)
- `health-check.md` → `src/health/health.check.ts` (`ComponentType.HEALTH_CHECK`)
...
**ТОЛЬКО:** `service.ts.md` → `src/modules/{feature}/{feature}.service.ts` (`LLMGenerationPoint.SERVICE_METHODS`)
